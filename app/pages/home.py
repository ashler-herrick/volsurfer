...
with st.expander("Underlying and DTEs"):
    ...
    strike_width_input = st.number_input(
        "Strike Width",
        min_value=1,
        max_value=None,
        value=init_session_state("strike_width", 5),
    )
...
with st.expander("Underlying & Simulation Parameters"):
    elapsed_time = st.slider(
        ...
        min_value=0.1,
        max_value=60.0,
        value=init_session_state("elapsed_time", 10.0),
        step=0.1,
    )
    timesteps = st.slider(
        "Timesteps",
...
with st.expander("Future Vol Surface (Optional)"):
    new_atm_vols_input = st.text_input(
        "New ATM Vols", init_session_state_csv("new_atm_vols", "")
    )
    new_skews_input = st.text_input(
        "New Skews", init_session_state_csv("new_skews", "")
    )
    new_kurtosis_input = st.text_input(
        "New Kurtosis", init_session_state_csv("new_kurtosis", "")
    )
...
if st.button("Submit"):
    ...
    st.session_state["new_atm_vols"] = (
        parse_csv(new_atm_vols_input) or st.session_state["atm_vols"]
    )
    st.session_state["new_skews"] = (
        parse_csv(new_skews_input) or st.session_state["skews"]
    )
    st.session_state["new_kurtosis"] = (
        parse_csv(new_kurtosis_input) or st.session_state["kurtosis"]
    )
    ...
    if len(set(length_check)) != 1:
        st.error(
            "Error: The lists for DTEs, ATM Vols, Skews, Kurtosis, New ATM Vols, New Skews, and New Kurtosis must all have the same length."
        )
    else:
