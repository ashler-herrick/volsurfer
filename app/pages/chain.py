...
try:
    ...
    for i, d in enumerate(unique_dtes):
        with tabs[i]:
            ...
            call_df = df[(df["DTE"] == d) & (df["Type"] == "Call")].reset_index(
                drop=True
            )
            ...
            call_grid_response = AgGrid(
                call_df,
                gridOptions=call_grid_options,
                height=400,
            ...
            put_grid_response = AgGrid(
                put_df,
                gridOptions=put_grid_options,
                height=400,
                fit_columns_on_grid_load=True,
            )
            ...
            selected_options = pd.concat(
                [selected_calls, selected_puts], ignore_index=True
            )
            ...
            if not selected_options.empty:
                ...
                for (
                    index,
                    row,
                ) in selected_options.iterrows():  # Correctly unpack row data
                ...
                if st.button(f"Add Selected Options for DTE {d} to Portfolio"):
                    for index, row in selected_options.iterrows():  # Correct loop
                        option_key = (
                            f"{row['Type']}_Strike{row['Strike']}_DTE{row['DTE']}"
                        )
                        contract_size = st.session_state["contract_sizes"].get(
                            option_key, 1
                        )  # Get stored contract size
                        ...
                        st.session_state["portfolio_options"].append(
                            {
                                ...
                                "underlying_price": st.session_state[
                                    "underlying_price"
                                ],
                            }
                        )
    ...
    st.write(
        "Input parameters on the homepage under 'Vol Surface and Simulation Parameters' before selecting options from the chain."
    )
